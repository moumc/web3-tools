...
async function collectToken(account, tokenInfo, targetAddress, rpcClient, logger) {
  ...
  try {
    ...
    // 构建 transfer 数据
    // ERC20 transfer(address,amount) 需要 32 字节地址 + 32 字节金额
    const addressHex = targetAddress.slice(2).padStart(64, '0');
    const amountHex = balance.toString(16).padStart(64, '0');
    const data = ERC20_TRANSFER_SELECTOR + addressHex + amountHex;
 ...
 * 归集原生币 (ETH, BNB 等)
 * @param {Object} account - 账户 {address, privateKey}
 * @param {string} nativeSymbol - 原生币符号 (ETH, BNB)
 * @param {string} targetAddress - 目标地址
 * @param {Object} rpcClient - RPC 客户端
 * @param {Logger} logger - 日志实例
 * @returns {Promise<CollectResult>} 归集结果
 */
async function collectNative(account, nativeSymbol, targetAddress, rpcClient, logger) {
  const { address, privateKey } = account;
  const tokenName = nativeSymbol;

  try {
    const balance = await rpcClient.getNativeBalance(address);
 ...
 * @returns {Promise<Array<CollectResult>>} 归集结果列表
 */
async function collectNativeCoins(accounts, nativeSymbol, targetAddress, rpcClient, logger) {
  logger.info(`=== 开始归集 ${nativeSymbol} ===`);
  logger.info(`目标地址: ${targetAddress}`);
  ...
  for (const account of accounts) {
    const result = await collectNative(account, nativeSymbol, targetAddress, rpcClient, logger);
    results.push(result);
  }
...
export { collectTokens, collectNativeCoins };
